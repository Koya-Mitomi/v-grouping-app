...
export async function submitTeam(page: string, limit: string, sorted: string, formData: FormData) {
  ...
  if (!teamName) {
    redirect(`/events/${eventId}?page=${page}&limit=${limit}&sorted=${sorted}&message=${encodeURIComponent('チーム名は必須です')}`);
  }
  ...
  redirect(`/events/${eventId}?page=${page}&limit=${limit}&sorted=${sorted}&message=${encodeURIComponent(message)}`);
}
