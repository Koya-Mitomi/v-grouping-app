...
return (
    <div className="w-full max-w-4xl py-4 md:py-10 flex flex-col items-center gap-4">
      <form action={submitTeam} className="w-full flex flex-col items-center gap-6">
        <TeamName
          initialTeamName={defaultValues.teamName}
          eventId={eventId}
          memberIds={memberIds.split(',').map(str => parseInt(str, 10))}
          teamId={teamId}
        />
        <div className="w-full overflow-x-auto rounded-xl border border-gray-200 shadow-sm bg-white">
          <table className="w-full text-left border-collapse min-w-[500px]">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th colSpan={5} className="px-6 py-4 text-sm text-center font-semibold text-gray-600">
                  チームメンバー一覧
                </th>
              </tr>
        ...
        <SubmitButton label="保存" className="w-full max-w-xs px-6 py-3 bg-green-500 text-white font-bold rounded-lg hover:bg-green-600 cursor-pointer disabled:bg-gray-400 disabled:cursor-not-allowed text-center transition-all shadow-md" />
      </form>
    </div>
  )
}
