          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th colSpan={2} className="px-6 py-4 text-sm text-center font-semibold text-gray-600 whitespace-nowrap">編集 / 削除</th>
              </tr>
            </thead>
              ...
              {displayList.length > 0 ? (
                displayList.map((player) => (
                  <tr key={player.id} className="transition-colors">
                    <td className="px-6 py-4 text-sm text-center font-medium text-gray-900 whitespace-nowrap">{player.name}</td>
