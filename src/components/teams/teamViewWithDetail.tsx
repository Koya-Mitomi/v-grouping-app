...
export const TeamViewWithDetail = (props: { teamName: string; teamMembers: Player[] }) => {
  ...
  return (
    <div className="w-full max-w-[26rem] h-[30rem] rounded-xl border border-blue-200 p-3 shadow-lg flex flex-col bg-white resize overflow-auto">
      ...
      <div className="space-y-1 flex-1 overflow-y-auto pr-1">
        {teamMembers.length > 0 ? (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className="text-gray-800 font-bold truncate text-sm md:text-base">{member.name}</span>
                </div>
                <div className="mt-1 flex flex-wrap gap-1">
                  ...
                  <span className="inline-flex items-center rounded-full bg-gray-100 px-2 py-0.5 text-[10px] md:text-[11px] font-medium text-gray-600">
                    {member.gender}
                  </span>
                </div>
              </div>
          ...
          <p className="text-gray-500 text-center py-8 text-sm italic">メンバー未登録</p>
        )}
      </div>
    </div>
  )
